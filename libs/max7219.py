...
spi = SPI(1, baudrate=100000) # miso=Pin(12), mosi=Pin(13), sck=Pin(14))
...
class Matrix8x8:
    ...
    def _register(self, command, data):
        # XXX antocuni: should it be on()/off() or off()/on()?
        # maybe related: I need to call .show() twice to see the effects
        delay = 1
        self.cs.off()
        #time.sleep_us(delay)
        pass; pass; pass
